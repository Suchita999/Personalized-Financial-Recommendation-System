...
def chart_savings_impact_breakdown(income, expenses, years=30):
    ...
    fig.add_annotation(
        x=1, y=recommended_final,
        text=f'Total Impact: +${additional_wealth:,.0f}',
        showarrow=True,
        arrowhead=2,
        arrowsize=1,
        arrowwidth=2,
        arrowcolor='#8ac35a',
        font=dict(size=12, color='#8ac35a'),
        ax=-80,
        ay=-60
    )
...
def main():
    ...
    with st.expander("⚙️  Adjust projection parameters", expanded=False):
        ...
        with cc1:
            horizon = st.selectbox("Time horizon", [10, 15, 20, 25, 30, 35, 40], index=4)
        with cc2:
    ...
    kpis = [
        ("Monthly Income",    f"${monthly_income:,.0f}",  "gross, pre-tax",            "#8ac35a"),
        ("Current Savings",   f"${monthly_sav:,.0f}/mo",  f"Rate: {sr:.1%}",            "#5a9c8a"),
        ("Target Monthly Δ",  f"${gap_monthly:,.0f}" if gap_monthly > 0 else "✓ On Track", 
         f"Already exceeding target!" if gap_monthly == 0 else f"to reach {savings_pct} rate", "#d4b84a"),
        (f"{horizon}-Yr Wealth", f"${projected_30yr/1e6:.2f}M" if projected_30yr>1e6 else f"${projected_30yr:,.0f}",
         f"at {effective_target_rate:.0%} rate", "#8ac35a"),
        ("E-Fund Gap",        f"${max(expenses*0.25-savings,0):,.0f}",
                              f"≈{ef_months_away:.0f} months away",           "#c05a5a"),
        ("Family Size",       f"{family} {'person' if family==1 else 'people'}",
                              bracket.replace(' Families','').replace(' Households',''), "#5a9c8a"),
    ]
    ...
    current_projection = _compound(0, current_monthly, 0.06, time_horizon)[-1]
    recommended_projection = _compound(0, recommended_monthly, 0.07, time_horizon)[-1]
    ...
    with analysis_col3:
        time_saved = max(current_time_to_goal - recommended_time_to_goal, 0)
        st.markdown(f"""
        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.02); border-radius: 12px;">
            <div style="font-size: 0.8rem; color: rgba(240,237,230,0.4); margin-bottom: 0.5rem;">Time Saved</div>
            <div style="font-family: 'Playfair Display', serif; font-size: 1.3rem; color: #d4b84a; margin-bottom: 0.3rem;">
                {time_saved:.1f} years
            </div>
            <div style="font-size: 0.85rem; color: rgba(240,237,230,0.5);">
                {(time_saved*12):.0f} months
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with progress_col1:
        ...
        current_progress = min(current_projection / goal_amount * 100, 100) if goal_amount > 0 else 0
        recommended_progress = min(recommended_projection / goal_amount * 100, 100) if goal_amount > 0 else 0
        
        st.markdown(f"""
        <div style="margin: 1.5rem 0;">
            <div style="font-size: 0.9rem; color: rgba(240,237,230,0.6); margin-bottom: 0.8rem;">
                Progress toward {goal_type} in {time_horizon} years
            </div>
            <div style="background: rgba(255,255,255,0.05); border-radius: 10px; height: 24px; overflow: hidden; margin-bottom: 0.5rem;">
                <div style="display: flex; height: 100%;">
                    <div style="background: {RED}; width: {current_progress}%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; color: white;">
                        {current_progress:.0f}%
                    </div>
                    <div style="background: {GREEN}; width: {max(recommended_progress - current_progress, 0)}%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; color: white;">
                        {recommended_progress - current_progress:.0f}%
                    </div>
                </div>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 0.75rem; color: rgba(240,237,230,0.4);">
                <span>Current: ${current_projection:,.0f}</span>
                <span>Recommended: ${recommended_projection:,.0f}</span>
                <span>Goal: ${goal_amount:,.0f}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with progress_col2:
        ...
        monthly_needed_current = goal_amount / (time_horizon * 12)
        monthly_needed_recommended = goal_amount / (time_horizon * 12)
        
        st.markdown(f"""
        <div style="text-align: center; padding: 1rem; background: rgba(138,195,90,0.06); border-radius: 12px;">
            <div style="font-size: 0.8rem; color: rgba(240,237,230,0.4); margin-bottom: 0.5rem;">Monthly Required</div>
            <div style="font-family: 'Playfair Display', serif; font-size: 1.1rem; color: #8ac35a; margin-bottom: 0.3rem;">
                ${monthly_needed_current:,.0f}
            </div>
            <div style="font-size: 0.75rem; color: rgba(240,237,230,0.5);">
                to reach goal in {time_horizon} years
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # ── CHART ROW 2: Budget + Emergency Fund ──
    st.markdown("""
    <div class="section-head">
    ...
    with r2:
        st.markdown("""
        <div class="chart-card">
            <div class="chart-title">Emergency Fund Build-Up</div>
            <div class="chart-desc">
                Time to reach 3-month and 6-month expense cushion at your recommended savings pace.
            </div>
        """, unsafe_allow_html=True)
        fig4 = chart_emergency_fund(expenses, monthly_sav)
    ...
    # ── SUMMARY CALLOUT ──
    diff_30 = max(_compound(0, target_monthly, ret_rate, horizon)[-1] - \
                  _compound(0, monthly_sav,    ret_rate, horizon)[-1], 0)

    # Dynamic messaging based on savings performance
    if current_savings_rate >= target_rate:
        summary_title = "You're already on an excellent financial path!"
        summary_amount = f"${projected_30yr/1e6:.2f}M" if projected_30yr>=1e6 else f"${projected_30yr:,.0f}"
        summary_text = f"With your impressive {current_savings_rate:.0%} savings rate, you're on track to build"
        summary_detail = f"Continuing your current strategy at {assumed_return} annual return. You're already exceeding typical recommendations!"
    else:
        summary_title = "Following FinWise's plan could add"
        summary_amount = "$" + f"{diff_30/1e6:.2f}M" if diff_30 >= 1e6 else "$" + f"{diff_30:,.0f}"
        summary_text = "to your wealth over {horizon} years."
        summary_detail = f"Based on increasing your monthly savings by ${gap_monthly:,.0f} and investing at {assumed_return} annual return. Past performance does not guarantee future results."

    st.markdown(f"""
    <div style="
        background: linear-gradient(135deg, rgba(138,195,90,0.08), rgba(90,156,138,0.06));
        border: 1px solid rgba(138,195,90,0.2);
        border-radius: 20px; padding: 2rem 2.5rem; margin: 2rem 0;
        text-align: center;
    ">
        <div style="font-family:'Playfair Display',serif; font-size:clamp(1.4rem,3vw,2.2rem);
                    color:#f0ede6; margin-bottom:.5rem;">
            {summary_title}
            <span style="color:#8ac35a;">
                {summary_amount}
            </span>
            {summary_text if 'already' in summary_title.lower() else ''}
        </div>
        <div style="font-size:.88rem; color:rgba(240,237,230,.4); max-width:520px; margin:0 auto;">
            {summary_detail}
        </div>
    </div>
    """, unsafe_allow_html=True)
